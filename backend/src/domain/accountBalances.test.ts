...
test("sem movimento não há série", () => {
  assert.deepEqual(monthlyCheckingSeries([open("nu")], []), []);
});

test("saldo inicial entra desde o primeiro mês", () => {
  const s = monthlyCheckingSeries([open("nu", 10_000)], [d("nu", "2026-01", 500)]);
...
test("o saldo é acumulado, não o delta do mês", () => {
  const s = monthlyCheckingSeries(
    [open("nu")],
    [d("nu", "2026-01", 1000), d("nu", "2026-02", 200), d("nu", "2026-03", -500)],
  );
  assert.deepEqual(values(s), [1000, 1200, 700]);
});

test("mês sem movimento nenhum não abre buraco na série", () => {
  const s = monthlyCheckingSeries([open("nu")], [d("nu", "2026-01", 1000), d("nu", "2026-04", 100)]);
...
test("a série atravessa a virada do ano", () => {
  const s = monthlyCheckingSeries([open("nu")], [d("nu", "2026-11", 100), d("nu", "2027-02", 100)]);
  assert.deepEqual(s.map(p => p.ym), ["2026-11", "2026-12", "2027-01", "2027-02"]);
});

test("contas somam", () => {
  const s = monthlyCheckingSeries(
    [open("nu", 1000), open("inter", 2000)],
    [d("nu", "2026-01", 100), d("inter", "2026-01", 50)],
  );
...
test("conta encerrada sai da soma no mês do encerramento — inclusive", () => {
  const s = monthlyCheckingSeries(
    [open("nu"), shut("inter", "2026-03")],
    [
      d("nu", "2026-01", 1000), d("inter", "2026-01", 500),
      d("nu", "2026-02", 0),
      d("nu", "2026-04", 0),
    ],
  );
  // jan/fev: 1000 + 500 · mar em diante: só o Nubank
  assert.deepEqual(values(s), [1500, 1500, 1000, 1000]);
});

test("o histórico ANTES do encerramento continua contando a conta morta", () => {
  const s = monthlyCheckingSeries(
    [shut("inter", "2026-03", 800)],
    [d("inter", "2026-01", 200), d("inter", "2026-04", 0)],
  );
  assert.deepEqual(values(s), [1000, 1000, 0, 0]);
});

test("encerrar sem transferência de saída não deixa saldo fantasma", () => {
  // O jeito certo: o dinheiro sai pro Nubank antes de fechar. Depois do
  // encerramento os R$ 5 continuam sendo seus — no Nubank.
  const comTransferencia = monthlyCheckingSeries(
    [open("nu"), shut("inter", "2026-03")],
    [
      d("inter", "2026-01", 500),
      d("nu", "2026-02", 500), d("inter", "2026-02", -500),
      d("nu", "2026-03", 0),
    ],
  );
  ...
  // ele, o acumulado carregaria os R$ 5 do Inter pra sempre.
  const semTransferencia = monthlyCheckingSeries(
    [open("nu"), shut("inter", "2026-03")],
    [d("inter", "2026-01", 500), d("nu", "2026-03", 0)],
  );
...
test("conta aberta depois entra sem alterar o passado", () => {
  const s = monthlyCheckingSeries(
    [open("nu"), open("novo")],
    [d("nu", "2026-01", 1000), d("novo", "2026-03", 700)],
  );
