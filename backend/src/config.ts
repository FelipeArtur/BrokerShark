...
export const accountByStatementFormat = (f: StatementFormat): AccountConfig | undefined =>
  config().accounts.find(a => a.statementFormat === f);

export const accountByInvoiceFormat = (f: InvoiceFormat): AccountConfig | undefined =>
  config().accounts.find(a => a.invoiceFormat === f);
