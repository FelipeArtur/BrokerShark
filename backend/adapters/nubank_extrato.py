...
- valor > 0, "banco inter" + "joao da silva" → skip (already in inter-db)
- valor > 0, any                    → income (is_revenue=1)
- valor < 0, "banco inter" + "joao da silva" → transfer to inter-db
- valor < 0, any                    → expense

Note: transfers "joao da silva" without "banco inter" are salary from a previous
employer deposited at BancoExemplo/BB — treated as real income (is_revenue=1).
"""
...
def _classify(description: str, valor: float) -> ParsedRow:
    ...
    if valor > 0:
        if "joao da silva" in desc and "banco inter" in desc:
            return ParsedRow(
                row_type="skip",
