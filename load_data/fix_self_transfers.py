...
What this does:
  1. nu-db → inter-db outgoing expenses: sets dest_account_id='inter-db'
     (identified by 'BANCO INTER' + 'JOAO DA SILVA' in description)
  2. inter-db → nu-db outgoing expenses: sets dest_account_id='nu-db'
     (identified by 'JOAO DA SILVA' in description on inter-db)
  ...
  4. Marks nu-db income from external self-transfers with counterpart='SELF'
     (from BancoExemplo, BB, PicPay, Easynvest — stays as income for balance,
...
with sqlite3.connect(config.DB_PATH) as conn:
    ...
    r1 = conn.execute("""
        ...
        WHERE account_id = 'nu-db'
          ...
          AND description LIKE '%JOAO DA SILVA%'
    """)
    ...
    # 4. Mark nu-db income from external self-transfers (BancoExemplo, BB, etc.)
    r4 = conn.execute("""
        UPDATE transactions
...
with sqlite3.connect(config.DB_PATH) as conn:
    remaining_income_self = conn.execute(
        "SELECT COUNT(*) FROM transactions WHERE flow='income' AND description LIKE '%JOAO DA SILVA%' AND counterpart IS NULL"
    ).fetchone()[0]
    print(f"  income entries with JOAO DA SILVA and counterpart IS NULL: {remaining_income_self}  (expected: 0)")

    unlinked_outgoing = conn.execute(
        "SELECT COUNT(*) FROM transactions WHERE flow='expense' AND description LIKE '%JOAO DA SILVA%' AND dest_account_id IS NULL"
    ).fetchone()[0]
