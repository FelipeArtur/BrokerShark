...
- **Caixinha/Porquinho derivadas:** vivem só no ledger (não vão p/ tabela `investments` nem B3). `analytics.get_ledger_savings_positions()` = `Σ(aplicações) − Σ(resgates)` por keyword de poupança (`rdb`/`caixinha` → Caixinha; `porquinho` → Porquinho), por banco, sem SELF. `/api/investments` anexa (`id=null, derived=true`). **Nunca usar keyword de corretora (NuInvest/Tesouro)** — viraria posição B3 e contaria 2x.
