/* quick-entry.js — Web-based transaction entry sidebar + CSV ImportModal */
/* global React, postTransaction, postIncome, postInvestmentMovement,
          postImportCsvPreview, postImportCsvConfirm, fetchExpenseCategories */
...
function ExpenseForm({ onSubmit, onCancel }) {
  ...
  return h("form", { onSubmit: handleSubmit, style: { display: "flex", flexDirection: "column", gap: 14 } },
    h(FieldRow, { label: "Pagamento", id: "ef-account" },
      h("select", { id: "ef-account", className: "select", value: accountKey, onChange: e => setAccountKey(e.target.value), style: { height: 32 } },
    ...
    h(FieldRow, { label: "Valor", hint: installHint, id: "ef-amount" },
      h(CurrencyInput, { id: "ef-amount", value: amount, onChange: setAmount, autoFocus: true, large: true })
    ),
    ...
    h(FieldRow, { label: "Descrição", id: "ef-desc" },
      h("input", { id: "ef-desc", className: "input", placeholder: "Ex.: iFood, PS Store", value: description, onChange: e => setDescription(e.target.value), style: { height: 36 } })
    ),
...
function IncomeForm({ onSubmit, onCancel }) {
  ...
  return h("form", { onSubmit: handleSubmit, style: { display: "flex", flexDirection: "column", gap: 14 } },
    ...
    h(FieldRow, { label: "Valor", id: "if-amount" }, h(CurrencyInput, { id: "if-amount", value: amount, onChange: setAmount, autoFocus: true, large: true })),
    !isTransfer && h(FieldRow, { label: "Descrição (opcional)", id: "if-desc" },
      h("input", { id: "if-desc", className: "input", placeholder: type === "salary" ? "Salário maio" : "Detalhe da entrada", value: description, onChange: e => setDescription(e.target.value), style: { height: 36 } })
    ),
    h(FieldRow, { label: "Data" }, h(DateChooser, { value: date, onChange: setDate })),
...
function InvestmentForm({ onSubmit, onCancel }) {
  ...
  return h("form", { onSubmit: handleSubmit, style: { display: "flex", flexDirection: "column", gap: 14 } },
    ...
    h(FieldRow, { label: "Destino", id: "inv-dest" },
      h("select", { id: "inv-dest", className: "select", value: destination, onChange: e => setDestination(e.target.value), style: { height: 32 } },
        INVESTMENTS_OPTIONS.map(o => h("option", { key: o.name, value: o.name }, o.name))
      )
    ),
    h(FieldRow, { label: "Valor", id: "inv-amount" }, h(CurrencyInput, { id: "inv-amount", value: amount, onChange: setAmount, autoFocus: true, large: true })),
    h(FieldRow, { label: "Observação (opcional)", id: "inv-obs" },
      h("input", { id: "inv-obs", className: "input", placeholder: "Ex.: aporte mensal", value: desc, onChange: e => setDesc(e.target.value), style: { height: 36 } })
    ),
    h(FieldRow, { label: "Data" }, h(DateChooser, { value: date, onChange: setDate })),
    h("div", { style: { display: "flex", gap: 6, marginTop: 4 } },
      h("button", { type: "button", className: "btn", onClick: onCancel, style: { flex: 1 } }, "Cancelar"),
...
/* ── ImportModal ──────────────────────────────────────────────────────────── */
const { useEffect: _imEf, useRef: _imRef } = React;
