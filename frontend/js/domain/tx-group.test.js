...
test("net: investimento é aplicação menos resgate, e bate com o rodapé", () => {
  const gs = G.buildGroups([
    tx({ id: 1, flow: "expense", method: "transfer", category_id: null, amount: 1000 }),
    tx({ id: 2, flow: "income", method: "transfer", is_revenue: 0, category_id: null, amount: 4000 }),
  ], cats);
...
test("espécie sem categoria NÃO cai em 'Sem categoria' — vira grupo próprio", () => {
  ...
  const gs = G.buildGroups([
    tx({ id: 1, category_id: null, category: null }),
    tx({ id: 2, method: "transfer", category_id: null }),
    tx({ id: 3, counterpart: "SELF", method: "transfer", category_id: null }),
    tx({ id: 4, is_settlement: 1, category_id: null }),
    tx({ id: 5, is_third_party: 1, category_id: null }),
  ], cats);
...
test("categorias primeiro (por total desc), espécies no fim", () => {
  const gs = G.buildGroups([
    tx({ id: 1, method: "transfer", category_id: null, amount: 99999 }),
...
test("alvo em centavos vira reais uma vez, na fronteira", () => {
  ...
  const g = gs[0];
  assert.equal(g.budget, 1200);
  assert.equal(g.prevSpent, 1000);
...
test("groupDelta: fração vs. mês anterior; sem base é null", () => {
  ...
  assert.equal(G.groupDelta(gs[0]), 0.5);
...
test("scaleFor: respeita piso e teto, cresce com a fatia", () => {
  ...
  assert.equal(G.scaleFor(500, 0), G.SCALE_MIN);
  const mid = G.scaleFor(500, 1000);
  assert.ok(mid > G.SCALE_MIN && mid < G.SCALE_MAX);
  assert.equal(G.scaleFor(9999, 1000), G.SCALE_MAX);
});
