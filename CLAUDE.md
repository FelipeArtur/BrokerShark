...
backend/
  ...
  dashboard/server.py      # Flask + Waitress (12 threads cfg, SSE, idle auto-shutdown)
frontend/
...
- **Runtime: foreground, resource-minimal (deploy em rethink — ver TODOS T-C).** Dashboard roda em foreground via **`./run.sh`** (`main.py` bloqueia no `waitress.serve`, logs no terminal). **Footprint:** ~0% CPU ocioso (threads bloqueiam, não giram), ~43 MB RSS vivo, **zero** parado. `DASHBOARD_THREADS` (12) e **auto-shutdown por ociosidade** (`IDLE_SHUTDOWN_MIN`=30; 0 desabilita) — quando todas as abas fecham (zero clientes SSE via `events.client_count()`) e não chega request por X min, o watchdog dá SIGINT gracioso em si mesmo e libera a RAM. Aba aberta (SSE vivo) nunca dispara; qualquer request reseta o relógio. Gate: `tests/integration/test_idle_shutdown.py`. Conscientemente **sem serviço always-on** — o dono quer consumo mínimo, só roda quando usa. Backup amarrado a **abrir o app** (`backup.request_startup_snapshot()` no boot, change-gated/off-thread — ver Automated Jobs) + refresh pós-import; sem scheduler. Avulso manual ainda dá: `PYTHONPATH=backend .venv/bin/python -m jobs.backup`. **Restore seguro: `python -m jobs.restore`** (wrapper com guard fail-closed: recusa se o dashboard está servindo na porta — restaurar sob o writer vivo corrompe; `--latest`/caminho/picker/`--list`; confirma; falha fechada sem TTY salvo `--yes`). Miolo em `core/backup.py::restore_backup` (verify + sidecar `.pre-restore` + swap atômico). Parar/subir o app ainda é manual (Ctrl+C no `./run.sh` → restore → `./run.sh`). O modelo antigo (systemd user units + linger + OnFailure alert) está no `git log` (commit `08db96c`~) caso o rethink de runtime decida ressuscitá-lo.
...
statement_coverage (id, year, month, origin, created_at)   -- UNIQUE(year,month); origin: import | manual
              -- meses "contabilizados" p/ a rede de segurança (extrato importado, incl. vazio, ou dismiss manual)
```
...
- **Histórico** = "o que aconteceu". Seletor de meses com dados, 4 métricas (número + Δ vs média), gráfico fluxo 6m (`DualLine`), por categoria, Top PIX (lado a lado), tabela filtrável (flow · método · categoria · conta · busca) com categorização inline e filtro "Sem categoria". **Rede de segurança de importação:** banner + flag no strip de meses listam meses passados **sem nenhum lançamento** (extrato esquecido) e cutucam se o mês atual segue vazio após o dia 5. Detecção é frontend (`primitives.js::findMonthGaps`/`currentMonthMissing`/`fmtMonthGaps` sobre `/api/monthly?present=1`); buraco = mês no intervalo [primeiro dado, mês atual) ausente da lista present **E não coberto**. O selo na Home (`OverviewView`) reusa os mesmos helpers. **Cobertura (`statement_coverage`, `/api/statement-coverage`):** evita falso-positivo de mês legitimamente vazio. Um mês conta como coberto se (a) um import o cobriu — o front deriva o período de **cada nome de arquivo** (`coverageFromFilename`: Nubank `NU_..._01AGO2021_31AGO2021`, Inter `Extrato-01-06-2026-a-24-06-2026`) ∪ datas das linhas (`coverageFromISODates`) e grava `origin='import'` no confirm, **incluindo arquivo vazio** (header-only, ex. AGO2021); ou (b) o usuário clicou **"sem movimento"** num chip do banner (`origin='manual'`). Account-agnostic, idempotente (UNIQUE(year,month); import nunca é rebaixado por manual). Gate: `tests/integration/test_coverage.py`.
