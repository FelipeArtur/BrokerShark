...
- **B3 = tabela verdade (posições de corretora).** Full-sync por `match_key` (ISIN/código/ticker). Abas presentes num relatório mais novo sinalizam quais tipos de posição existem — aba ausente → soft-close das posições daquele tipo. CDBs Inter = Porquinho (`group_name='Porquinho'`).
