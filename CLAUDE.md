...
- **Segurança de rede:** bind em `127.0.0.1` + guard de Host/Origin em `server.py` (DNS-rebinding/CSRF) — a API não tem auth, então isso é load-bearing. Reforços (VibeSec): rejeita `Sec-Fetch-Site: cross-site` (defense-in-depth do Origin); headers `nosniff`/`X-Frame-Options DENY`/CSP/`Permissions-Policy`/no-store em `/api/`; CDN com SRI; **DB SQLite chmod 0600** no `init_db` (`_restrict_db_permissions` — sem auth, perms de arquivo são a fronteira at-rest); upload cap 8MB + B3 cap 16MB comprimido **e 200MB descomprimido** (anti zip-bomb, in-memory sem zip-slip, openpyxl sem XXE); `bulk_categorize` dedupe+cap (10k)+chunk (≤900, sob o limite de vars do SQLite). SQL 100% parametrizado (f-strings só interpolam fragmentos estáticos de cláusula). Gate: `tests/integration/test_hardening.py`.
