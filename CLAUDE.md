...
- **Categorização = manual, com sugestão suggest-only:** importados entram com `category_id=NULL` → filtro "Sem categoria" + edição inline no Histórico (`<select>` por linha → `PATCH /api/transactions/<id>`). Sem **auto-aplicação** por regras. O preview de import mostra um `<select>` inline pré-selecionado com uma categoria **sugerida do histórico** (mesmo comerciante categorizado antes): `analytics.get_categorized_history()` → `domain.classification.build_category_index()` indexa `(flow, merchant_key) → (category_id, name)` (mais frequente, desempate pela mais recente; `merchant_key` tira acentos/dígitos/tokens estruturais do extrato); só linhas `is_categorizable` recebem sugestão (mirror da consumption/revenue-rule — leg de transfer/SELF/investimento nunca). **Suggest-only:** o `category_id` da staging row **nunca** é auto-escrito; troca manual persiste na hora (`PATCH /api/import/staging`), sugestão intocada só é gravada **no confirm** e só p/ linhas incluídas. Gate: `tests/integration/test_import_suggestions.py`.
