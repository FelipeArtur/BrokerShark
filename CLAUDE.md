...
- **Segurança de rede:** bind em `127.0.0.1` + guard de Host/Origin em `server.py` (DNS-rebinding/CSRF) — a API não tem auth, então isso é load-bearing. Reforços (VibeSec): rejeita `Sec-Fetch-Site: cross-site` (defense-in-depth do Origin); headers `nosniff`/`X-Frame-Options DENY`/`Permissions-Policy`/no-store em `/api/`; **CSP `script-src 'self'`** (libs vendorizadas, sem CDN, sem Babel/`unsafe-eval`); **DB SQLite chmod 0600** no `init_db` (`_restrict_db_permissions` — sem auth, perms de arquivo são a fronteira at-rest); upload cap 8MB + B3 cap 16MB comprimido **e 200MB descomprimido** (anti zip-bomb, in-memory sem zip-slip, openpyxl sem XXE); `bulk_categorize` dedupe+cap (10k)+chunk (≤900, sob o limite de vars do SQLite). SQL 100% parametrizado (f-strings só interpolam fragmentos estáticos de cláusula). Gate: `tests/integration/test_hardening.py`.
...
| Frontend | React 18 + Chart.js, **vendorizados localmente** (`frontend/js/vendor/`, sem CDN), **sem build step e sem Babel** — código é JS puro hyperscript (`React.createElement`, nunca JSX). Cada arquivo de app é envelopado em IIFE (escopo próprio; sem colisão de `const` no escopo global). |
